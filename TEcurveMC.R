# this code computes Monte-Carlo point estimates of the TE(s1) curve in Figure 1 using the NP-TE estimator and saves them 
...
for (i in 1:length(pi)){
  TE.MCestimates <- sapply(1:nMC, function(seed, s1grid, n, beta, pi, truncateMarker){ getEstVE(s1grid, n, beta, pi, truncateMarker, seed) }, s1grid=s1, n=n, beta=beta, pi=pi[i], truncateMarker=truncateMarker)
  # save the matrix 'TE.MCestimates' as an .RData file
  save(TE.MCestimates, file=file.path(currentDir, paste0("TE_MCestimates_nMC=",nMC,"_N=",n,"_truncateMarker=",truncateMarker,"_pi=",pi[i],".RData")))
}
